...
int Last(double mas[], int size, int i = 0, int last = -1) { // Функція для знаходження індексу останнього додатного елемента масиву
    if (i == size) {
        return last;
    }
    if (mas[i] > 0) {
...
double Sum(double mas[], int last, int i = 0, double sum = 0) { // Функція для обчислення суми елементів до останнього додатного
    if (i == last) {
        return sum;
    }
    sum += mas[i];
...
void NULLS(double mas[], int size, double a, double b, int i = 0) { // Функція для заміни елементів в інтервалі [a, b] на нулі
    if (i == size) {
        return;
    }
...
int main() {
    ...
    double* mas = new double[n]; // Динамічне створення масиву
